...
export class HttpClient {
	...
	async #parseSuccess<T>(response: Response): Promise<SendflowlyResponse<T>> {
		...
		// Unwrap the `{ data, pagination?, meta? }` envelope into the SDK's
		// response shape. Pagination is well-typed (PaginationMeta); meta is
		// endpoint-specific and passed through as `unknown` for resource
		// methods (e.g., analytics) to cast.
		if (isSuccessEnvelope(json)) {
			...
			const base = { data: envelope.data as T, error: null as null, headers }
			const withPagination =
				envelope.pagination !== undefined ? { ...base, pagination: envelope.pagination } : base
			return envelope.meta !== undefined
				? { ...withPagination, meta: envelope.meta }
				: withPagination
		}
