...
- f5e0797: Fix email response types to match the actual wire format (camelCase)

  `EmailListItem`, `EmailEvent`, and `EmailDetail` previously declared snake_case fields (`from_address`, `created_at`, …) that the API never returns — responses are camelCase (`fromAddress`, `createdAt`). Code written against the old types compiled but read `undefined` at runtime.

  Breaking type changes:
  - `EmailListItem`: all fields renamed to camelCase; removed `organization_id`, `in_reply_to`, `references`, `conversation_id`, `message_id_header` (the list endpoint never returned them)
  - `EmailEvent`: renamed to camelCase; removed `email_id` (not present in detail events)
  - `EmailDetail`: no longer `extends EmailListItem` — it now mirrors the API's detail projection exactly (adds `htmlBody`, `textBody`, `inReplyTo`, `references`, `tags`, `events`, `attachments`; omits `sesMessageId`, `apiKeyName`, `attachmentCount`)
  - New `EmailDetailAttachment` type for `EmailDetail.attachments`

  Request types are unchanged — they were always correctly snake_case (`reply_to`, `page_size`). No runtime behavior changed; this is purely aligning the declared types with what the API actually sends.

## 0.1.0-beta.7

### Patch Changes

- 07e3073: Comprehensive README update — covers all 6 v1 resources with runnable examples.

  The previous README only documented `emails.send()` + webhook verification (the only resources shipped at the time of the initial publish). With Phase 2 complete and all six resources (`emails`, `domains`, `webhooks` CRUD, `templates`, `suppressions`, `analytics`) live on npm, the README needed to match the actual surface so users discover what's available without reading source code.

  Additions:
  - Per-resource sections with realistic copy-paste examples
  - Resource summary table near the top with anchor links
  - Full re-exported type list (32 types across all resources)
  - Clear separation between `sendflowly.webhooks` (CRUD on the main client) and `@sendflowly/sdk/webhooks` (signature verifier sub-path)
  - Analytics section documents the `meta.range.clamped` pattern for plan-retention detection
  - Suppressions section documents the auto-add-on-bounce behavior (so users don't manually add for hard bounces / complaints)

  Also updates `packages/sdk/state.md` to mark Phase 2 resource coverage as complete and record the actual shipped versions (`0.1.0-beta.2` through `.6`).

  No code changes; doc + state update only.
